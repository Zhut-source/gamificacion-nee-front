...
export interface Card {
  ...
  symbol: string;
  name: string;
  isConditional: boolean; // TRUE: ==, !=, >, <, >=, <=, &&, ||  | FALSE: +, -, *, /, %
  isFlipped: boolean;
  isMatched: boolean;
  isHint: boolean;
}
...
export interface BucketItem {
  ...
  guessedConditional: boolean; // true si el alumno dijo SÍ, false si dijo NO
}
...
export class CondicionalesComponent implements OnInit, OnChanges{
  ...
  conditionalOperators = [
    ...
    { symbol: '&&', name: 'Y lógico (AND)' },
    { symbol: '||', name: 'O lógico (OR)' }
  ];
  ...
  initGame(difficulty: 'easy' | 'medium' | 'hard') {
    ...
    if (difficulty === 'medium') { totalPairs = 3; this.gridColumns = 3; }
    if (difficulty === 'hard') { totalPairs = 4; this.gridColumns = 4; }
  ...
  generateCards(totalPairs: number) {
    ...
    const shuffledCond = [...this.conditionalOperators].sort(() => 0.5 - Math.random());
    const shuffledMath = [...this.mathOperators].sort(() => 0.5 - Math.random());

    for (let i = 0; i < condCount; i++) selectedOperators.push({...shuffledCond[i], isConditional: true});
    for (let i = 0; i < mathCount; i++) selectedOperators.push({...shuffledMath[i], isConditional: false});

    this.cards = [];
    let idCounter = 1;

    selectedOperators.forEach(op => {
      this.cards.push({ id: idCounter++, symbol: op.symbol, name: op.name, isConditional: op.isConditional, isFlipped: false, isMatched: false, isHint: false });
      this.cards.push({ id: idCounter++, symbol: op.symbol, name: op.name, isConditional: op.isConditional, isFlipped: false, isMatched: false, isHint: false });
    });
  ...
  // --- LÓGICA DE JUEGO (FLIP) ---

  flipCard(card: Card) {
    if (this.isProcessing || card.isFlipped || card.isMatched || this.activePairToClassify || !this.isPlaying) return;
  ...
  validateFinalResult() {
    ...
    if (errors === 0) {
      ...
      this.gameResult.emit({ status: 'success', message: `¡Clasificación perfecta! Has dominado las condicionales.` });
    } else {
      this.gameStatus = 'failed';
      this.audioService.playSound('robot-off');
      this.gameResult.emit({ status: 'failed', message: `Encontraste los pares, pero clasificaste ${errors} operador(es) incorrectamente. ¡Inténtalo de nuevo!` });
    }
  }
  ...
  mostrarPista() {
    ...
    if (this.firstCard) {
      const pairCard = hiddenCards.find(c => c.symbol === this.firstCard!.symbol);
      if (pairCard) {
